...
use crate::day4::matrix::{bool::BoolMatrix, Matrix, MutMatrix};
...
fn is_update_correctly_ordered(update: &[usize], page_ordering_graph: &BoolMatrix) -> bool {
    for (i, current_page) in update.iter().enumerate() {
        for following_page in update[i+1..].iter() {
            if page_ordering_graph.get(*current_page, *following_page).unwrap() {
                return false;
            }
        }
    }
...
fn is_transitive(matrix: &BoolMatrix) -> bool {
    ...
    for i in 0..n {
        for j in 0..n {
            for k in 0..n {
                if matrix.get(j, i).unwrap() && matrix.get(k, j).unwrap() && !matrix.get(k, i).unwrap() {
                    return false;
                }
            }
        }
